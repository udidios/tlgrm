...
function apiSearch() {
    ...
    if (input.value.length > 0) {
        fetch(`https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=` + encodeURI(input.value), {
            headers: {
                'X-API-KEY': '1c547473-0385-454b-a351-8c41f27ea5fd'
            }
        })
            .then(res => res.json())
            .then(function (data) {
                ...
                for (var key in data['items']) {
                    var rating = "<div style=\"position: absolute; top: 10px; left: 6px; display: flex; width: 30px; font-size: .75rem; color: var(--text-color);; border-radius: .4rem; justify-content: center;background: #3bb33b;\">\
                            <span>" + data['items'][key]['rating'] + "</span>\
                          </div>";
                    var div = "\
                    <a onclick=\"apiItem('film/" + data["items"][key]["filmId"] + "')\">\
                        <div class=\"creditsCardWrapper\">\
                            <img alt=\"" + data["items"][key]["nameRu"] + "\" src=\"" + data["items"][key]["posterUrl"] + "\" loading=\"lazy\">\
                            <div style=\"position: absolute; top: 165px; left: 5px; display: flex; font-size: .75rem; color: var(--text-color); border-radius: .4rem; justify-content: center; background: #f60;\">\
                                <span style=\"margin: 0px 5px;\">" + data['items'][key]['year'] + "</span>\
                            </div>\
                            " + rating + "\
                            <div class=\"description\">\
                                <h2 class=\"title\">" + data["items"][key]["nameRu"] + "</h2>\
                            </div>\
                        </div>\
                    </a>\
                    ";
